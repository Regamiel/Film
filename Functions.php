...
function show_row($film) {
    echo "<tr><td><a href='details.php?film=" . $film->id . "'>".$film->title."</td><td>" . $film->type . "</td></tr>";    
}
...
/**
 * Classe contenat les propriétés d'un film
...
class Finder {
    ...
    public function __construct($data){
        ...
        //méthode pôur trouver un film à partir de son ID
    ...
    //Si rien, on renvoi tous les films
    else {
        foreach ($this->_data as $row){
            if ($row[1] !="title") {
                $found[] = new Film($row);
